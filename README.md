...
- Web 客户端：完成 Sonora 注册登录、个人资料、喜欢歌曲、默认“我喜欢的音乐”、自建歌单和流媒体播放接入。
...
1. 测试音乐数据补齐：导入更多歌曲、封面和歌单关联，让客户端首页可完整演示。
2. 播放历史：把“最近播放”接入用户维度真实记录。
3. 歌单体验细化：补移动端管理入口、歌曲排序和更友好的封面上传。
4. 运营内容：首页内容配置、歌词独立编辑。
5. 稳定性：补充后端单元测试、前端类型检查、启动脚本和环境示例。
